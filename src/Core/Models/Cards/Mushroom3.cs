...
/// <summary>
/// 蘑菇3：给自己 4（0）层孢子，获得 1 层缓冲。
/// 1 费，token 稀有度，目标自身，消耗，食物牌。
/// 由【蘑菇盲盒】随机生成，不会出现在卡池中。
/// </summary>
public sealed class Mushroom3 : CardModel, IFoodCard
{
	// 卡面图片（文件名与卡牌 ID 不一致，需显式指定）
	public override string PortraitPath => ImageHelper.GetImagePath("packed/card_portraits/scout/mushroom_3.png");

	protected override string PortraitPngPath => ImageHelper.GetImagePath("packed/card_portraits/scout/mushroom_3.png");
	...
	protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
	{
		...
		await PowerCmd.Apply<BufferPower>(
			choiceContext,
			base.Owner.Creature,
			base.DynamicVars["BufferPower"].BaseValue,
			base.Owner.Creature,
			this
		);
	}
	...
	public static async Task<IEnumerable<CardModel>> CreateInHand(Player owner, int count, bool isUpgraded, ICombatState combatState)
	{
		...
		{
			Mushroom3 mushroom = combatState.CreateCard<Mushroom3>(owner);
			if (isUpgraded)
			{

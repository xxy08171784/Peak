...
<<<<<<< HEAD
		// 2. 计算实际需要扣减的层数（至多 7 层）
		int layersToDecrease = Math.Min(Amount, 7);
...
<<<<<<< HEAD
		// 4. 使用官方最推荐的 HittableEnemies 获取当前所有可被击中的敌人
		List<Creature> aliveEnemies = Owner.CombatState.HittableEnemies.ToList();

		if (aliveEnemies.Count > 0)
		{
			// 5. 视觉抛光：在每一个敌人身上播放受击斩击特效（与 FestivePopper 相同的群体表现！）
			VfxCmd.PlayOnCreatureCenters(aliveEnemies, "vfx/vfx_attack_slash");

			// 6. 调用底层伤害指令
			// 提示：如果你希望这个伤害无视敌人的格挡值，
			// 可以把 ValueProp.Unpowered 改为 ValueProp.Unpowered | ValueProp.Unblockable

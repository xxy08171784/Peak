...
/// MyClimbing → MountaineeringExpert（我的攀登 → 登山专家）。
...
public static class TouchOfOrobasRefinementPatch
{
	...
	{
		// 重新构造完整映射：先包含原版 5 对，再追加 Scout 的 MyClimbing → MountaineeringExpert
		var upgrades = new Dictionary<ModelId, RelicModel>
		{
			...
			{ ModelDb.Relic<MyClimbing>().Id, ModelDb.Relic<MountaineeringExpert>() }
		};
		__result = upgrades;
		return false;
	}
